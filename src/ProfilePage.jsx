...
export default function ProfilePage({ userVotes, tracks, onViewUser, onUpload, onOpenSettings, onOpenStorefront }) {
  ...
  useEffect(() => {
    ...
    fetch(`${SUPABASE_URL}/rest/v1/purchases?buyer_username=eq.${encodeURIComponent(currentUser.username)}&select=*&order=purchased_at.desc`, {
      headers: { apikey: ANON_KEY, Authorization: `Bearer ${ANON_KEY}` }
    }).then(r => r.json()).then(data => {
                          ...
                          <div style={{ marginTop: '10px' }}>
                            ...
                            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                              {[
                                ...
                                { value: 'purple', label: 'Purple', bg: 'linear-gradient(135deg,#9b59b6,#6a0dad)', color: '#fff' },
                                { value: 'teal', label: 'Teal', bg: 'linear-gradient(135deg,#00ced1,#20b2aa)', color: '#fff' },
                                { value: 'crimson', label: 'Crimson', bg: 'linear-gradient(135deg,#dc143c,#8b0000)', color: '#fff' },
                                { value: 'lime', label: 'Lime', bg: 'linear-gradient(135deg,#32cd32,#00ff00)', color: '#000' },
                                { value: 'cyan', label: 'Cyan', bg: '#00f5ff', color: '#000' },
                                { value: 'sunset', label: 'Sunset', bg: 'linear-gradient(135deg,#ff6b6b,#ffa500)', color: '#fff' },
                                { value: 'forest', label: 'Forest', bg: 'linear-gradient(135deg,#228b22,#00aa00)', color: '#fff' },
